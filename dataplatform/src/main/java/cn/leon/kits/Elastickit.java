...
import cn.leon.domain.PageData;
...
import cn.leon.util.ConditionUtil;
...
public class Elastickit {
  ...
  @Autowired private ElasticSearchPoolUtil elasticSearchPoolUtil;
  @Autowired private ParseUtil parseUtil;

  @Autowired private ConditionUtil conditionUtil;
  ...
  public boolean createIndex(IndexForm form) {
    ...
    CreateIndexRequest indexRequest =
        new CreateIndexRequest(index).alias(new Alias(index.split("-")[0]));
          ...
          builder.startObject(entry.getKey());
          {
            String name =
                Objects.nonNull(entry.getValue())
                    ? entry.getValue().getClass().getTypeName()
                    : "java.lang.String";
    ...
    indexRequest
        .settings(
            Settings.builder()
                .put("index.number_of_shards", form.getShard())
  ...
  public int insertDocument(EsStorageForm esStorageForm) {
    ...
    try {
      ...
      UpdateRequest updateRequest =
          new UpdateRequest(esStorageForm.getBizName(), esStorageForm.getStorageId())
              .doc(condition)
              .upsert(condition);
  ...
  public int insert(EsStorageForm esStorageForm) {
    ...
    try {
      client = elasticSearchPoolUtil.getClient();
      Map<String, Object> condition = esStorageForm.getCondition();
      IndexRequest indexRequest = new IndexRequest(esStorageForm.getBizName()).source(condition);
  ...
  public void insertDocuments(List<EsStorageForm> list) {
    ...
    list.forEach(
        esStorageForm -> {
          bulkProcessor.add(
              new IndexRequest(esStorageForm.getBizName()).source(esStorageForm.getCondition()));
        });
    bulkProcessor.flush();
    // 关闭
  ...
  public PageData<Map<String, Object>> searchDocument(SearchForm form) {
    ...
    SearchRequest searchRequest =
        new SearchRequest().indices(form.getBizName().concat("-*")).source(sourceBuilder);
    //                .scroll(scroll);
    ...
    //        }
    elasticSearchPoolUtil.returnClient(client);
    PageData<Map<String, Object>> pageData =
        PageData.<Map<String, Object>>builder()
  ...
  public CountResultVo countIndexByCodition(SearchForm searchForm) {
    ...
    CountRequest countRequest =
        new CountRequest().indices(searchForm.getBizName().concat("-*")).source(sourceBuilder);
  ...
  public String searchDocumentId(SearchForm form) {
    ...
    BoolQueryBuilder qb = searchAction(form);
    SearchSourceBuilder sourceBuilder =
        new SearchSourceBuilder().query(qb).timeout(new TimeValue(60, TimeUnit.SECONDS));
    SearchRequest searchRequest =
        new SearchRequest()
            .indices(form.getBizName().toLowerCase().concat("-*"))
  ...
  private void getSortType(Map<String, String> sort, SearchSourceBuilder searchSourceBuilder) {
    ...
    for (Map.Entry<String, String> entry : sort.entrySet()) {
      searchSourceBuilder.sort(
          entry.getKey(), entry.getValue().equals("desc") ? SortOrder.DESC : SortOrder.ASC);
    }
  }
}
