...
//            "mapper.xml.ftl"
    ...
    private String getFilePath(String moduleRoot, String domainName, String ftl) {
        ...
        switch (ary[1]) {
            case "java":
                dirPath = moduleRoot + GeneratorConstants.MAIN_JAVA + GeneratorHelper.converterPackage(config.getRootPackage()) + "/" + ary[0] + "/";
                break;
            case "xml":
                dirPath = moduleRoot + GeneratorConstants.MAIN_RESOURCES + GeneratorHelper.converterPackage(config.getXmlPackage()) + "/";
                break;
            default:
